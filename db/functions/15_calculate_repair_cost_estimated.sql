-- ================================================================
-- REPAIR COST CALCULATION (ESTIMATED)
-- Requirement: "Cout_diag et Cout_rep" - Section "Résultat par device"
-- ================================================================
CREATE OR REPLACE FUNCTION calculate_repair_cost_estimated(
    p_device_id UUID,
...
BEGIN
    ...
    IF p_stage = 'pre_test' THEN
        ...
        -- "Cout-reparation_est = moyenne Cout-reparation_reel sur 90 derniers jours ouvrés"
        SELECT AVG(dab.repair_cost)
        INTO v_cost
        ...
        AND dab.repair_cost IS NOT NULL;
        
    ELSIF p_stage = 'post_test' THEN
        -- Requirement: "Après mise en test (étape 2)"
        -- "Cout-reparation_est = Taux horaire collaborateur x temps moyen rep sur 90j"
        ...
        SELECT dssc.repair_duration
        INTO v_avg_duration
        FROM device_service_sub_categories dssc
        WHERE dssc.id = v_subcat_id;
        
        v_cost := COALESCE(v_user_hourly_cost, 15) * COALESCE(v_avg_duration, 1.5);
    END IF;
